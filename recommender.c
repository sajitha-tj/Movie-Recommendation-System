...
void recommender(int uid){
	...
	get_utility_matrix(utility_matrix, PATH_RATINGS_LEARN, No_of_movies, No_of_users, userid); //getting utility matrix which will contain ratings according to user id(row) and movie id(coloumn)
	get_movie_names(movienames, PATH_MOVIES); //getting movie names according to movie id(index)
	get_movie_genres(moviegenres, PATH_MOVIES_GENRES); //getting movie genres according to movie id(index)
